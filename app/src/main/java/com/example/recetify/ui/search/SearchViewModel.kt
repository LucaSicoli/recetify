...
class SearchViewModel(
    ...
    fun toggleSave(recipeId: Long) = viewModelScope.launch {
        ...
        _savedIds.update { set -> if (currently) set - recipeId else set + recipeId }
        runCatching {
            if (currently) api.unsaveRecipe(recipeId) else api.saveRecipe(recipeId)
        }.onFailure {
            _savedIds.update { set -> if (currently) set + recipeId else set - recipeId }
        }
    }
